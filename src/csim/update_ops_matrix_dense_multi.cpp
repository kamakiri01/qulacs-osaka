...
// target_qubit_index_list, UINT target_qubit_index_count, const CTYPE* matrix,
// CTYPE* state, ITYPE dim); void
// multi_qubit_dense_matrix_gate_old_parallel(const UINT*
// target_qubit_index_list, UINT target_qubit_index_count, const CTYPE* matrix,
...
void multi_qubit_dense_matrix_gate(const UINT* target_qubit_index_list,
    ...
    if (target_qubit_index_count == 1) {
        ...
        // multi_qubit_dense_matrix_gate_old_single(target_qubit_index_list,
        // target_qubit_index_count, matrix, state, dim);
        // multi_qubit_dense_matrix_gate_old_parallel(target_qubit_index_list,
        // target_qubit_index_count, matrix, state, dim);
        ...
        // target_qubit_index_count, matrix, state, dim); return;
...
void multi_qubit_dense_matrix_gate_single(const UINT* target_qubit_index_list,
    UINT target_qubit_index_count, const CTYPE* matrix, CTYPE* state,
    ITYPE dim) {
    UINT sort_array[64];
    ITYPE mask_array[64];
...
cursor++) { UINT insert_index = sorted_insert_index_list[cursor]; basis_0 =
insert_zero_to_basis_index(basis_0, 1ULL << insert_index, insert_index);
                }
...
#ifdef _OPENMP
void multi_qubit_dense_matrix_gate_old_parallel(const UINT*
target_qubit_index_list, UINT target_qubit_index_count, const CTYPE* matrix,
CTYPE* state, ITYPE dim) {
...
buffer_list + thread_id * matrix_dim;
